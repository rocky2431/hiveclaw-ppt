...
## 6. Wedge: HR-agent for One-person Departments

Hive 最适合从高频、可衡量、可复用的“一人部门”切入。但 Hive 不预设客户的部门划分——具体做什么部门由客户决定。Hive 提供的是 HR-agent 入口和底层治理层，让客户能自己创建并保留这些 Agent。

### 6.1 客户自己定义部门，Hive 提供创建路径

| 阶段 | 客户体验 | Hive 提供 |
|---|---|---|
| Hire | 部门负责人和 HR-agent 对话，定义岗位目标、职责、协作风格 | HR-agent / `create_digital_employee` 工具 |
| Identity | 生成 `soul.md` 身份契约，进入企业租户，绑定权限和 IM 通道 | soul 引擎 / 权限矩阵 / 通道接入 |
| Work & Learn | Agent 进入真实工作流，记忆和判断逐步沉淀 | T0/T2/T3 记忆管线 / Dream / Heartbeat |
| Govern | 工具调用经过 Security Zone、Capability Policy、Approval Flow | 治理 + 审计层 |
| Inherit | 员工离开或转岗，Agent + 记忆 + 工作状态留在公司 | 资产归属和交接机制 |

具体 Agent 是客服、市场、销售运营、招聘协调还是财务助理——这是客户自己的事。Hive 不出售部门 Agent，出售的是让客户能创建、治理、继承这些 Agent 的基础设施。

### 6.2 试点交付方式

每个早期客户不卖“平台大而全”，只交付一条 HR-agent 路径下可跑通的部门闭环：

- 1 个负责人；
- N 个由客户用 HR-agent 创建的部门 Agent；
- 1 套 soul 配置；
- 1 套 skill pack；
- 3-5 个 trigger 配方；
- IM 通道接入；
...
| Workflow Packs | 按行业 pack、高级工具和深度集成收费 | 复用客户已沉淀的工具与配置，提升毛利和复用率 |
...
### 0-6 个月：HR-agent 落地，客户用 HR-agent 创建部门 Agent

目标：

- 完成 3 个付费客户；
- 客户用 HR-agent 通过对话创建他们自己的部门 Agent；
- 每位客户沉淀 soul 配置、skill pack、trigger 配方、审批策略和 ROI 看板；
- 完成至少 1 次“负责人交接测试”案例。
...
### 6-18 个月：从 HR-agent 到 Workflow 中台

目标：

- 将客户验证过的工具组合打包为 Workflow Packs；
- 建立 skill pack、trigger recipe 的复用体系；
...
本轮融资应围绕“让 HR-agent 把技术骨架交付给首批客户”使用。
...
| 试点交付 | 30% | 做深 3 个客户用 HR-agent 创建 Agent 的端到端交付，形成可复用交付手册 |
